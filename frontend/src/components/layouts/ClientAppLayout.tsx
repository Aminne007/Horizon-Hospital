  ...
  const links = [
    {
      ...
      icon: (
        <path
          fill="currentColor"
          d="M4 4h7v7H4V4Zm9 0h7v11h-7V4ZM4 13h7v11H4V13Zm9 13V17h7v9h-7Z"
        />
      ),
    },
    {
      ...
      icon: (
        <path
          fill="currentColor"
          d="M6 4h12a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2Zm0 2v14h12V6H6Zm2 3h8v2H8V9Zm0 4h8v2H8v-2Z"
        />
      ),
    },
    {
    ...
    <div className="min-h-screen bg-[#E8F4F8] lg:pl-28">
      ...
      {profileOpen && (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/20 px-4 py-6 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-[28px] border border-[#d3e8f2] bg-white p-6 text-[#1f3d47] shadow-2xl shadow-[#4A90A4]/15">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#3e6f7b]">Logged in</p>
                <h3 className="text-xl font-black">{profile.full_name || "Client"}</h3>
                <p className="text-sm text-[#527884]">{profile.email || "No email on file"}</p>
              </div>
              <button
                type="button"
                onClick={() => setProfileOpen(false)}
                className="rounded-full border border-[#d3e8f2] bg-[#E8F4F8] px-3 py-1 text-xs font-semibold text-[#2f5f6a] shadow-sm"
              >
                Close
              </button>
            </div>
            <div className="mt-4 space-y-2 text-sm">
              <div className="flex items-center justify-between rounded-2xl bg-[#f6fbfd] px-4 py-3 ring-1 ring-[#d3e8f2]">
                <span className="text-[#3e6f7b]">Role</span>
                <span className="font-semibold text-[#1f3d47]">{profile.role || "CLIENT"}</span>
              </div>
              <div className="flex items-center justify-between rounded-2xl bg-white px-4 py-3 ring-1 ring-[#d3e8f2]">
                <span className="text-[#3e6f7b]">User ID</span>
                <span className="font-mono text-xs font-semibold text-[#1f3d47]">{profile.id || "Unavailable"}</span>
              </div>
            </div>
            {profileLoading && <p className="mt-3 text-xs text-[#3e6f7b]">Refreshing profile...</p>}
